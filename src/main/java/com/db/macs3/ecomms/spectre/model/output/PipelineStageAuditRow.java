 ...
 *
 * <p>Field name note: {@link #getPipelineExecId} maps to the BQ column literally named {@code pipeline_exec_id}
 * (with an "x") — see {@code BqColumns.PipelineStageAudit#PIPELINE_EXEC_ID}; kept verbatim from the table.
 *
 * <p><b>NOT NULL columns:</b> {@link #getComposerDagName}, {@link #getComposerDagPath}, {@link #getDprocScriptName}
 * and {@link #getDprocScriptPath} are NOT NULL in the table. Neither {@code RuntimeArgs} nor {@code DataprocConfig}
 * carries real values for them, so {@code ScanEngineJobRunner.writeStageAudit} passes a placeholder string for each.
 */
public class PipelineStageAuditRow implements Serializable {
     ...
     * @param errorCount        INTEGER {@code error_count}
