 ...
 * {@code feature-hit-summary} {@link Row}, skipping errored results — see
 ...
 */
public final class FeatureHitSummaryRowMapper implements MapPartitionsFunction<MessageProcessingResult, Row> {
    ...
    public Iterator<Row> call(Iterator<MessageProcessingResult> input) {
        ...
        while (input.hasNext()) {
            ...
            if (!result.isError()) {
                rows.add(OutputTableWriter.toRow(result.getFeatureHitSummaryRow()));
            }
        }
